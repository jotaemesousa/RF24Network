...
int main(void)
{
	...
	while (1) {
		...
		if (role == role_ping_out)
		{
			...
			bool ok = radio.write( &time, sizeof(int16_t ));
			...
			else
			{
				...
				int16_t got_time;
				radio.read( &got_time, sizeof(int16_t));

				// Spew it
				UARTprintf("Got response %x, round-trip delay: %u\n\r",got_time,millis()-got_time);
			}
		...
		if ( radio.available() )
			{
				...
				while (!done)
				{
					...
					done = radio.read( &got_time, sizeof(int16_t) );

					// Spew it
					UARTprintf("Got payload %x...",got_time);
				...
				radio.write( &got_time, sizeof(int16_t) );
